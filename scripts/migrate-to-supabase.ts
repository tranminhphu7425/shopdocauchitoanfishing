...
async function migrate() {
  ...
  console.log(
    `Found ${storeData.products?.length || 0} products and ${storeData.collections?.length || 0} collections.`,
  );
  ...
  storeDataString = storeDataString.replace(
    /\/images\/products\//g,
    publicUrlBase,
  );
  ...
  if (updatedStoreData.collections) {
    for (const collection of updatedStoreData.collections) {
      ...
      if (error) {
        console.error(
          `Error inserting collection ${collection.handle}:`,
          error,
        );
      } else if (data) {
