...
function parseOptionsFromTitle(title) {
  ...
  if (gamMatches && gamMatches.length > 1) {
    const values = gamMatches.map((m) =>
      m.trim().toLowerCase().replace("gam", "g"),
    );
    const uniqueValues = Array.from(new Set(values));
    if (uniqueValues.length > 1) {
...
async function run() {
  try {
    ...
    for (let i = 1; i < chunks.length; i++) {
      ...
      console.log(
        `   - ID: ${itemId} | Giá gốc: ${basePrice.toLocaleString()} ₫`,
      );

      // Tải và lưu ảnh sản phẩm
      const imageFilename = `${itemId}.jpg`;
      let localImgUrl =
        "/shopdocauchitoanfishing/images/products/placeholder.jpg";
      if (cdnImgUrl) {
      ...
      const lowercaseTitle = title.toLowerCase();
      const isPhuKien =
        lowercaseTitle.includes("lưỡi") ||
        lowercaseTitle.includes("day") ||
        lowercaseTitle.includes("khoen") ||
        lowercaseTitle.includes("cần") ||
        lowercaseTitle.includes("hộp");
      ...
      if (fs.existsSync(storePath)) {
        try {
          ...
          if (toAdd.length > 0) {
            ...
            console.log(
              `🎉 Đã đồng bộ thêm ${toAdd.length} sản phẩm mới vào: ${storePath}`,
            );
          } else {
            console.log(
              `ℹ️ Không có sản phẩm mới nào cần thêm vào: ${storePath} (Tất cả đã tồn tại)`,
            );
          }
        } catch (jsonErr) {
          console.error(
            `❌ Không thể cập nhật file ${storePath}:`,
            jsonErr.message,
          );
        }
      } else {
