...
export function GitHubStatusButton() {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("github-config-updated", syncConfig);
  }, []);
  ...
  return (
    <div className="relative">
      <button
        ...
        title={
          activeConfig ? "Trạng thái: Đã liên kết" : "Trạng thái: Chưa liên kết"
        }
      ...
      >
        ...
        <span
          className={`absolute top-1.5 right-1.5 h-2.5 w-2.5 rounded-full ring-2 ring-white dark:ring-neutral-900 ${activeConfig ? "bg-green-500" : "bg-amber-500 animate-pulse"}`}
        />
      </button>
        ...
        <div className="absolute right-0 top-14 z-50 w-72 rounded-2xl border border-neutral-200 bg-white p-4 shadow-xl dark:border-neutral-800 dark:bg-neutral-900 animate-in fade-in slide-in-from-top-2 duration-200">
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-2.5">
              <div
                className={`mt-1 h-2.5 w-2.5 rounded-full shrink-0 ${activeConfig ? "bg-green-500" : "bg-amber-500 animate-pulse"}`}
              />
                ...
                {activeConfig ? (
                  <div>
                    ...
                    <p className="text-[10px] text-neutral-500 dark:text-neutral-400">
                      Tự động đồng bộ dữ liệu sản phẩm & hình ảnh
                    </p>
                  </div>
                ) : (
                  <div>
                    ...
                    <p className="text-[10px] text-neutral-500 dark:text-neutral-400">
                      Vào trang Quản trị để nhập mã liên kết
                    </p>
                  </div>
                )}
              </div>
...
export function GitHubConfigModal() {
  ...
  const handleTest = async () => {
    ...
    if (res.success) {
      setStatusMessage({
        type: "success",
        text: "Mã hợp lệ! Kết nối thành công.",
      });
    } else {
  ...
  const handleSave = async () => {
    if (!config.token.trim()) {
      setStatusMessage({
        type: "error",
        text: "Vui lòng nhập Mã liên kết trước khi lưu!",
      });
      return;
    }
    ...
    if (!res.success) {
      setStatusMessage({
        type: "error",
        text: `Không thể lưu: ${res.message}`,
      });
      return;
    }
    ...
    <div className="mb-8 rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          <div
            className={`flex h-10 w-10 items-center justify-center rounded-xl font-bold text-white ${activeConfig ? "bg-green-600" : "bg-neutral-500"}`}
          >
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-bold text-lg text-neutral-900 dark:text-white">
                Liên kết với hệ thống để cập nhật thông tin sản phẩm
              </h3>
              {activeConfig ? (
